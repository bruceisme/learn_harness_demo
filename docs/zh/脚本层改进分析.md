...
| 章节 | 文件名 | 脚本层主要变化 |
|------|--------|---------------|
| chapter_01 | s01_agent_loop.py | 基础交互循环 |
| chapter_02 | s02_tool_use.py | 与 chapter_01 基本一致 |
| chapter_03 | s03_skill_loading.py | 与 chapter_01 基本一致 |
| chapter_04 | s04_todo_write.py | 与 chapter_01 基本一致 |
| chapter_05 | s05_subagent.py | 输出处理增强，彩色打印 |
| chapter_06 | s06_context.py | 上下文压缩状态保持 |
| chapter_07 | s07_permission_system.py | 权限模式选择，运行时命令 `/mode`、`/rules` |
| chapter_08 | s08_hook_system.py | 动态授权命令 `/allow` |
| chapter_09 | s09_memory_system.py | Memory 加载，`/memories` 命令 |
...
| chapter_18_2 | s18_v2_worktree.py | Worktree 信息展示 |
...
**演变说明**：
- 这四章的脚本层逻辑基本一致
- 主要变化发生在核心逻辑层（如 chapter_04 引入 TodoManager），但脚本层入口代码保持不变

---

### Capiter 5：输出处理增强

**文件**：`chapter_05/s05_subagent.py`
...
**文件**：`chapter_07/s07_permission_system.py`
...
**文件**：`chapter_08/s08_hook_system.py`
...
**文件**：`chapter_09/s09_memory_system.py`
...
**文件**：`chapter_18_2/s18_v2_worktree.py`
...
1. **阶段 1（chapter_01-4）**：基础交互循环
   - 简单的 `while input()` 同步循环
   - 无运行时命令，仅支持 `q`/`exit` 退出

2. **阶段 2（chapter_05-9）**：功能增强期
   - 彩色输出（chapter_05）
   - 上下文压缩状态保持（chapter_06）
   - 权限模式控制（chapter_07）
   - 动态授权（chapter_08）
   - 记忆系统（chapter_09）
   - 斜杠命令系统逐步建立
...
| 命令 | 引入章节 | 功能 |
|------|----------|------|
| `q`/`exit` | chapter_01 | 退出会话 |
| `/mode <mode>` | chapter_07 | 切换权限模式（default/plan/auto） |
| `/rules` | chapter_07 | 查看权限规则列表 |
| `/allow <path>` | chapter_08 | 动态授权目录写权限 |
| `/memories` | chapter_09 | 列出已加载的记忆 |
...
从 chapter_01 到 chapter_19_2，脚本层经历了从简单交互循环到复杂事件驱动架构的演变。核心改动包括：
