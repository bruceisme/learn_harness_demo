...
# -*- coding: utf-8 -*-
"""
s11_Resume_system.py - Resume System

Reference: learn-claude-code (https://github.com/shareAI-lab/learn-claude-code)
License: MIT License

This module introduces an error recovery and resume system for handling unexpected failures.
It demonstrates structured error handling across main loop, subagent loop, API calls, and tool execution.

Features:
    - Error handling: Catch and process errors in main loop, subagent loop, and API calls
    - Max tokens recovery: Automatic continuation when output limit is hit (up to 3 retries)
    - Prompt too long recovery: Auto-compact and retry when context exceeds limit
    - Connection backoff: Exponential backoff retry for network/rate limit errors
    - Structured error tracking: New data structure for error statistics
    - Graceful degradation: Handle various error types without crashing

Documentation:
    - Chinese: docs/zh/chapter_11/s11_Resume_system_文档.md
    - English: docs/en/chapter_11/s11_Resume_system_doc.md
"""
import os
    ...
    for attempt in range(MAX_RECOVERY_ATTEMPTS + 1):
        try:
            ...
            if "context_length_exceeded" in error_body or "maximum context length" in error_body:
                ...
                sys_msg = state.messages[0] if state.messages and state.messages[0].get("role") == "system" else None
...
def agent_loop(state: LoopState, compact_state: CompactState ) -> None:
    ...
    while True:
        if not any(m.get("role") == "system" for m in state.messages):
            state.messages.insert(0, {"role": "system", "content": prompt_builder.main_build()})
        else:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        todo_items = TODO.state.items
        status_tag = ""
        if todo_items:
            completed = sum(1 for it in todo_items if it.status == "completed")
            in_progress = [it for it in todo_items if it.status == "in_progress"]
            if in_progress:
                active_name = in_progress[0].active_form or in_progress[0].content
                # \x01 (Start ignore) 和 \x02 (End ignore) 是解决退格显示错误的核心
                status_tag = (
                    f"\x01\033[33m\x02[Todo {completed}/{len(todo_items)}"
                    f" | {active_name[:30]}{'…' if len(active_name) > 30 else ''}] \x01\033[0m\x02 "
                )
            elif completed < len(todo_items):
                status_tag = f"\x01\033[33m\x02[Todo {completed}/{len(todo_items)}] \x01\033[0m\x02 "

        prompt_str = f"\x01\033[2K\r\x02{status_tag}\x01\033[36m\x02s11 >> \x01\033[0m\x02"
