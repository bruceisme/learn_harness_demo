...
# -*- coding: utf-8 -*-
"""
s13_v2_backtask.py - Background Task with Parallel Subagent

Reference: learn-claude-code (https://github.com/shareAI-lab/learn-claude-code)
License: MIT License

This module extends s13_backtask.py with parallel subagent capability via background threads.
It demonstrates the background task dispatch pattern for concurrent task execution.

Features:
    - background_task: Spawn a subagent in background thread, returns task_id immediately
    - check_background: Query status of background subagents or shell commands
    - background_run: Execute shell commands in background threads
    - Non-blocking: Main agent continues while subagents work asynchronously
    - Notification queue: Completed background tasks push results to main agent context

Documentation:
    - Chinese: docs/zh/chapter_13/s13_v2_backtask_文档.md
    - English: docs/en/chapter_13/s13_v2_backtask_doc.md
"""
import os
...
CHILD_TOOLS = [
    ...
    {"type": "function","function": {"name": "background_run",
            "description": "Run a shell command in a background thread. Returns task_id immediately without blocking.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    {"type": "function","function": {"name": "check_background",
            "description": "Check status of a background task by task_id. Omit task_id to list all background tasks.",
            "parameters": {
                "type": "object",
                "properties": {
...
PARENT_TOOLS =  [
    ...
    {"type": "function","function": {"name": "background_task",
            "description": (
                ...
                "Use check_background to monitor progress or wait for <background-results>. "
...
def run_subagent(prompt: str) -> str:
    ...
    for step in range(30):  # safety limit
        # Drain background task notifications and inject as user message before LLM call
        notifs = BG.drain_notifications()
        if notifs and sub_state.messages:
            notif_text = "\n".join(
                f"[bg:{n['task_id']}] {n['status']}: {n['preview']} "
                f"(output_file={n['output_file']})"
                for n in notifs
            )
            sub_state.messages.append({"role": "user", "content": f"<background-results>\n{notif_text}\n</background-results>"})
...
def agent_loop(state: LoopState, compact_state: CompactState ) -> None:
    ...
    while True:
        ...
        if not has_next_step:
            ...
            if running:
                # 不退出，等通知，让 LLM 知道还有任务在跑
                state.messages.append({
                    "role": "user",
                    "content": f"[System] 还有 {len(running)} 个后台任务正在运行: {running}，请等待结果再回复用户。"
                })
                time.sleep(2)   # 稍等，给后台任务完成的机会
                # 继续循环 → 下一轮开头 drain 通知
            else:
                break
...
if __name__ == "__main__":
    ...
    while True:
        ...
        prompt_str = f"\x01\033[2K\r\x02{status_tag}\x01\033[36m\x02s13 >> \x01\033[0m\x02"
