...
This module introduces a memory system for persisting user preferences and project knowledge.
It demonstrates how to store cross-session information that cannot be derived from current work.

Features:
    - Memory types: Support for user, feedback, project, and reference memories
    - Privacy levels: Private (user-specific) and team (shared) memory separation
    - File-based storage: Each memory stored as a separate file
    - Selective persistence: Only store cross-session knowledge, not transient data
    - Memory filtering: Exclude file structures, task progress, temporary branches, and credentials

Documentation:
    - Chinese: docs/zh/chapter_09/s09_memory_system_文档.md
    - English: docs/en/chapter_09/s09_memory_system_doc.md
"""
import os
import os,json,re
import subprocess
...
class PermissionManager:
    ...
    def __init__(self, mode: str = None, rules: list = None):
        # Use environment variable or provided mode for non-interactive usage
        import os
        if mode is None:
