...
# -*- coding: utf-8 -*-
"""
s19_v2_mcp_plugin.py - Single Agent with MCP & Plugin System
...
Features:
    - MCP Client: stdio MCP client with JSON-RPC 2.0 protocol (connect/list_tools/call_tool/disconnect)
    - Plugin Loader: Discover plugins from .claude-plugin/plugin.json manifest
    - MCP Tool Router: Route mcp__{server}__{tool} prefixed calls to MCP servers
    - Capability Permission Gate: Unified permission gate for native and MCP tools
    - Risk Levels: read (read-only) / write (write operations) / high (destructive operations)
    - Permission Modes: default (ask for non-read) / auto (ask only for high risk)
    - Tool Pool Merge: Native tools prioritized, MCP tools appended with mcp__ prefix
    - Thread Safety: _call_lock protects MCP _send/_recv sequence

Documentation:
    - Chinese: docs/zh/chapter_19_2/s19_v2_mcp_plugin_文档.md
    - English: docs/en/chapter_19_2/s19_v2_mcp_plugin_doc.md
"""
import os
...
ALLOWED_PATHS: set = {WORKDIR}   # bash 路径白名单；用 /allow 命令在运行时扩展
...
class BackgroundManager:
    ...
    def check(self, task_id: str = None) -> str:
        ...
        for tid, t in self.tasks.items():
            progress_count = len(t.get("progress", []))
            prog_hint = f" [{progress_count} progress update(s)]" if progress_count else ""
            lines.append(
                f"{tid}: [{t['status']}]{prog_hint} {t['command'][:60]} "
                f"-> {t.get('result_preview') or '(running)'}"
            )
        return "\n".join(lines) if lines else "No background tasks."

    def report_progress(self, bg_task_id: str, message: str, step: str = "") -> str:
        """Called by subagent to post a progress milestone to the main agent."""
        with self._lock:
            if bg_task_id not in self.tasks:
                return f"Error: bg_task_id '{bg_task_id}' not found."
            entry = {
                "time": dt.now().strftime("%H:%M:%S"),
                "step": step,
                "message": message,
            }
            self.tasks[bg_task_id].setdefault("progress", []).append(entry)
        print(f"\033[35m[Progress {bg_task_id}] {step+': ' if step else ''}{message[:80]}\033[0m")
...
class MemoryManager:
    ...
    def load_memory_prompt(self) -> str:
        ...
        sections.append("")
        sections.append(
            "> Rules: Use `save_memory` to create, `update_memory` to modify, "
            "`delete_memory` to remove. "
            "Do NOT proactively delete memories unless the user explicitly requests it."
        )
        sections.append("")
    ...
    def update_memory(self, name: str, description: str = None,
                      mem_type: str = None, content: str = None) -> str:
        """Partially update an existing memory. Only provided fields are changed."""
        if name not in self.memories:
            return f"Error: memory '{name}' not found. Use save_memory to create it first."
        existing = self.memories[name]
        new_description = description if description is not None else existing["description"]
        new_type        = mem_type    if mem_type    is not None else existing["type"]
        new_content     = content     if content     is not None else existing["content"]
        if new_type not in MEMORY_TYPES:
            return f"Error: type must be one of {MEMORY_TYPES}"
        return self.save_memory(name, new_description, new_type, new_content)

    def delete_memory(self, name: str) -> str:
        """Delete a memory by name, removing its file and rebuilding the index."""
        if name not in self.memories:
            return f"Error: memory '{name}' not found."
        file_name = self.memories[name].get("file", "")
        if file_name:
            file_path = self.memory_dir / file_name
            try:
...
class SystemPromptBuilder:
    ...
    def _build_core(self) -> str:
        return (
            ...
            "2. DELEGATION: Use `background_task` to spawn subagents for actual coding, file editing, or shell commands. "
            "Multiple independent subagents can run in parallel. "
            "When writing `prompt`, always include: (a) exact goal, (b) relevant file paths, (c) acceptance criteria, (d) constraints/conventions. "
            "Use `context` for extra background (sibling task IDs, dependencies). "
            "Use `task_id` to bind the subagent to a specific task — it will auto-receive full task details. "
            "Monitor progress via `check_background(bg_task_id)` — subagents report milestones in real-time.\n"
            ...
            "8. MCP TOOLS: Tools prefixed with mcp__{server}__{tool} are provided by external MCP servers. "
            "Use them alongside native tools. They pass through the same permission gate before execution.\n"
        )
    ...
    def _build_sub_core(self) -> str:
        return (
            ...
            "7. PROGRESS: After completing each significant step, call `report_progress` with your `bg_task_id` (provided in the initial task message). "
            "Report what you did, what you found, and what comes next. This keeps the main agent informed in real-time without waiting for your final result.\n"
        )
    ...
    def _build_dynamic_context(self) -> str:
        ...
        try:
            ...
            if mcp_count:
                server_names = list(mcp_router.clients.keys())
                lines.append(f"MCP tools: {mcp_count} tool(s) from {len(server_names)} server(s): {', '.join(server_names)}")
        except Exception:
            pass
...
def run_update_memory(name: str, description: str = None,
                      mem_type: str = None, content: str = None) -> str:
    return memory_mgr.update_memory(name, description, mem_type, content)

def run_delete_memory(name: str) -> str:
    return memory_mgr.delete_memory(name)
...
TOOL_HANDLERS = {
    ...
    "report_progress": lambda **kw: BG.report_progress(kw["bg_task_id"], kw["message"], kw.get("step", "")),
    "save_memory":   lambda **kw: run_save_memory(kw["name"], kw["description"], kw["type"], kw["content"]),
    "update_memory": lambda **kw: run_update_memory(kw["name"], kw.get("description"), kw.get("type"), kw.get("content")),
    "delete_memory": lambda **kw: run_delete_memory(kw["name"]),
    "task_create": lambda **kw: TASKS.create(kw["subject"], kw.get("description", ""), kw.get("claim_role", "")),
    ...
    "background_task":  lambda **kw: run_subagent_background(kw["prompt"], kw.get("context", ""), kw.get("task_id")),  # [s13_v2 + context/task_id]
    # [s14 新增] cron 定时任务工具
...
CHILD_TOOLS = [
    ...
    {"type": "function","function": {"name": "report_progress",
        "description": "Report a progress milestone to the main agent. Call this after each significant step so the main agent can track your work in real-time.",
        "parameters": {
            "type": "object",
            "properties": {
                "bg_task_id": {
                    "type": "string",
                    "description": "Your background task ID (provided in the initial task message under 'Your Identity & Progress Reporting')"
                },
                "step": {
                    "type": "string",
                    "description": "Short step name, e.g. 'Step 1: Code analysis', 'Step 2: Implementation'"
                },
                "message": {
                    "type": "string",
                    "description": "What you did, what you found, and what you plan to do next"
                }
            },
            "required": ["bg_task_id", "message"]
        }
        }},
    ]
...
PARENT_TOOLS =  [
    ...
    {"type": "function","function": {"name": "save_memory",
            "description": "Create or overwrite a persistent memory that survives across sessions.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
    ...
    {"type": "function","function": {"name": "update_memory",
            "description": "Partially update an existing memory. Only the fields you provide are changed; omitted fields keep their current values.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
                    ...
                    "content": {
                        "type": "string",
                        "description": "New full content (optional)"
                    }
                },
                "required": ["name"]
            }
    ...
    {"type": "function","function": {"name": "delete_memory",
            "description": "Permanently delete a memory by name. Only call this when the user explicitly requests deletion.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
    ...
    {"type": "function","function": {"name": "background_task",
            "description": (
                ...
                "Returns bg_task_id immediately without blocking. "
                "The subagent will call report_progress() at each milestone so you can track it via check_background(). "
                "Results are automatically injected as <background-results> when done. "
                "Best for independent tasks that can run in parallel.\n"
                "PROMPT GUIDELINES: Include (1) exact goal, (2) relevant file paths, "
                "(3) acceptance criteria / how to verify success, (4) any constraints or conventions to follow."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
                        "type": "string",
                        "description": "Detailed task instructions. Must include: goal, relevant file paths, acceptance criteria, constraints."
                    },
                    "context": {
                        "type": "string",
                        "description": "Optional extra context: related task IDs, sibling subagent status, external dependencies, or background knowledge the subagent needs."
                    },
                    "task_id": {
                        "type": "integer",
                        "description": "Optional: bind to a task in the task manager. The subagent will automatically receive full task details (subject, description, status)."
                    }
                },
                "required": ["prompt"]
            }
...
def run_subagent(prompt: str, context: str = "",
                 task_id: int = None, bg_task_id: str = None) -> str:
    ...
    if task_id is not None:
        try:
            task_detail = TASKS.get(task_id)
            parts.append(f"## Bound Task (#{task_id})\n{task_detail}")
        except Exception:
            pass
    if context:
    ...
    if bg_task_id:
        parts.append(
            f"## Your Identity & Progress Reporting\n"
            f"Your background task ID is `{bg_task_id}`.\n"
            f"After completing each significant step, call `report_progress` to keep the main agent informed.\n"
            f"Required fields: bg_task_id='{bg_task_id}', step='<step name>', message='<what you did and what you found>'"
        )
    user_msg = "\n\n".join(parts)
    ...
    for step in range(30):  # safety limit
        ...
        if response_message.tool_calls:
            results,_,manual_compact,compact_focus = execute_tool_calls(
                response_message, interactive=False, track_tasks=False)
...
def run_subagent_background(prompt: str, context: str = "", task_id: int = None) -> str:
    """Spawn a subagent in a background thread. Returns bg_task_id immediately.
    The subagent result is pushed to BG notification queue when done.
    bg_task_id format: sub_xxxxxxxx
    """
    bg_task_id = "sub_" + str(uuid.uuid4())[:8]
    with BG._lock:
        BG.tasks[bg_task_id] = {
            "id": bg_task_id,
            "status": "running",
    ...
    print(f"\033[35m> Background Subagent {bg_task_id}: {prompt[:80]}...\033[0m")

    def _run():
        try:
            result = run_subagent(prompt, context=context, task_id=task_id, bg_task_id=bg_task_id)
            status = "completed"
        except Exception as e:
        ...
        with BG._lock:
            BG.tasks[bg_task_id]["status"] = status
            BG.tasks[bg_task_id]["result"] = result
            BG.tasks[bg_task_id]["finished_at"] = time.time()
            BG.tasks[bg_task_id]["result_preview"] = preview
    ...
    bound_hint = f" (bound to task #{task_id})" if task_id is not None else ""
    return (
        f"Background subagent {bg_task_id} started{bound_hint}: {prompt[:80]}\n"
        f"Use check_background('{bg_task_id}') to see real-time progress."
    )
...
def run_one_turn(state: LoopState,compact_state: CompactState) -> bool:
    ...
    for attempt in range(MAX_RECOVERY_ATTEMPTS + 1):
        try:
            response = client.chat.completions.create(
                    model=MODEL,
                    tools=build_mcp_tool_pool(PARENT_TOOLS),  # [s19_v2] 合并 MCP 工具
...
#     """从 bash 命令中提取所有绝对路径候选（以 / 开头的路径段）。"""
#     return _ABS_PATH_RE.findall(command)

def _extract_abs_paths(command: str) -> list[str]:
    """从 bash 命令中提取所有绝对路径候选（以 / 开头的路径段）。"""
    paths = _ABS_PATH_RE.findall(command)
    # 进一步过滤：排除以 '//' 开头的字符串（通常是 URL 残留或网络路径），只保留真实的绝对路径
    return [p for p in paths if not p.startswith("//")]

def _is_path_allowed(p: str) -> bool:
    """判断路径是否在 ALLOWED_PATHS 白名单内（子路径匹配）。"""
    try:
        target = Path(p)
        return any(
            target == allowed or target.parts[:len(allowed.parts)] == allowed.parts
            for allowed in ALLOWED_PATHS
        )
    except Exception:
        return True  # 路径解析失败时放行，避免误伤
    ...
    for tool_call in response_message.tool_calls:
        ...
        if f_name in TOOL_HANDLERS:
            ...
            try:
                ...
                print(f"\033[31m[PreHook error, continuing]: {_pre_err}\033[0m")
            ...
            # 1.5 bash 路径白名单检查：阻止访问 ALLOWED_PATHS 以外的路径
            if f_name == "bash":
                command_str = args.get("command", "")
                blocked_paths = [
                    p for p in _extract_abs_paths(command_str)
                    if not _is_path_allowed(p)
                ]
                if blocked_paths:
                    denied_list = ", ".join(blocked_paths[:5])
                    if interactive:
                        output_parts.append(
                            f"Permission denied: bash command accesses path(s) outside allowed workspace: "
                            f"{denied_list}. "
                            f"Use /allow <path> in the CLI to grant access, then retry."
                        )
                    else:
                        output_parts.append(
                            f"Permission denied: path(s) not in allowed workspace: {denied_list}. "
                            f"Ask the lead to run /allow <path> to grant access."
                        )
                    results.append({
                        "role": "tool",
                        "tool_call_id": tool_call.id,
                    ...
                    print(f"\033[31m  [PATH BLOCKED] {f_name}: {denied_list[:80]}\033[0m")
                    continue
            ...
            try:
                ...
                print(f"\033[31m[PostHook error, continuing]: {_post_err}\033[0m")
                post_result = {}
...
def _build_prompt_str() -> str:
    ...
    return f"\x01\033[2K\r\x02{status_tag}\x01\033[36m\x02s19v2 >> \x01\033[0m\x02"
...
if __name__ == "__main__":
    ...
    print("[s19_v2] Event-driven loop started. Cron tasks will fire without user input.")
