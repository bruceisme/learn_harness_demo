...
# -*- coding: utf-8 -*-
"""
s10_build_system.py - Build System

Reference: learn-claude-code (https://github.com/shareAI-lab/learn-claude-code)
License: MIT License

This module introduces a structured system prompt building mechanism.
It demonstrates how to organize instructions into fixed and dynamic sections for main and sub agents.

Features:
    - Structured prompts: Organize system instructions into fixed and dynamic groups
    - Core instructions: Base guidelines for agent behavior
    - Tool listing: Automatic tool documentation in prompts
    - Skill metadata: Include skill information in context
    - Memory section: Integrate memory into system prompts
    - CLAUDE.md chain: Load project-specific instructions
    - Dynamic context: Adapt prompts based on current state
    - Main/Sub agent differentiation: Separate instruction sets for different agent roles

Documentation:
    - Chinese: docs/zh/chapter_10/s10_build_system_文档.md
    - English: docs/en/chapter_10/s10_build_system_doc.md
"""
import os
...
class PermissionManager:
    ...
    def __init__(self, mode: str = "default", rules: list = None):
        if mode not in MODES:
            print(f"Unknown mode: {mode}. Choose from {MODES}. Default to default.")
            mode = "default"
...
class TodoManager:
    ...
    def update(self, items: list) -> str:
        ...
        for index, raw_item in enumerate(items):
            ...
            if not id:
                return f"Error: Item at index {index} missing 'id' field."
            if not content:
...
class SystemPromptBuilder:
    ...
    def _build_claude_md(self) -> str:
        ...
        # Subdirectory -- walk from cwd up to workdir, collecting CLAUDE.md files
        cwd = Path.cwd()
        if cwd != self.workdir and str(cwd).startswith(str(self.workdir)):
            subdir_claudes = []
            current = cwd
            while current != self.workdir and current != current.parent:
                subdir_claude = current /  + cfn + 
                if subdir_claude.exists():
                    subdir_claudes.append((current, subdir_claude.read_text()))
                current = current.parent
            for dp, ct in reversed(subdir_claudes):
                rp = dp.relative_to(self.workdir)
                sources.append((f"subdir ({rp}/CLAUDE.md)", ct))
        if not sources:
            return 
...
# Legacy constants - kept for reference, replaced by SystemPromptBuilder
# SYSTEM = f"""You are a coding agent at {WORKDIR}. ..."""
# SUBAGENT_SYSTEM = f"""You are a coding subagent at {WORKDIR}. ..."""
# MEMORY_GUIDANCE = """When to save memories: ..."""

# Legacy function - replaced by SystemPromptBuilder.main_build()
...
PARENT_TOOLS =  [
    ...
    {"type": "function","function": {"name": "todo",
        "description": "1.Use this tool to plan or update the current session plan for complex and multi-step tasks. Mark in_progress before starting, completed when done. Keep exactly one step in_progress when a task has multiple steps. 2.Do NOT immediately mark a task as 'completed' just because the subagent claims it is done. You MUST verify the subagent's work before calling the todo tool to mark it completed. If the work is flawed, explain the issue and spawn a new task to fix it.",
        "parameters": {
            "type": "object",
            "properties": {
                "items": {
...
def agent_loop(state: LoopState, compact_state: CompactState ) -> None:
    ...
    while True:
        ...
        """
        # mian_system = prompt_builder.build()#每次迭代都要加载不会很慢吗？
